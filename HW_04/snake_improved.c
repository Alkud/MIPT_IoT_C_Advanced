...
//
// 2. Написать функцию, которая будет проверять корректность выбранного направления.
...
int checkDirection(snake_t* snake, int32_t key)
{
    for (uint8_t i = 0; i < CONTROLS; ++i)
    {
        if (key == snake->controls[i].down && snake->direction == UP)
            return -1;
        if (key == snake->controls[i].up && snake->direction == DOWN)
            return -1;
        if (key == snake->controls[i].right && snake->direction == LEFT)
            return -1;
        if (key == snake->controls[i].left && snake->direction == RIGHT)
            return -1;
    }
    return 0;
}
